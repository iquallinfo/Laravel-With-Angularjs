...
schoex.controller('classesController', function(dataFactory,$rootScope,$scope) {
  ...
  dataFactory.httpRequest('classes/listAll').then(function(data) {
	...
	$scope.allsubjects = data.subjectlist;
    showHideLoad(true);
  });
  ...
  $scope.checkToggle = function(){
	   $scope.albumNameArray = [];
		angular.forEach($scope.teachers, function(teacher){
		  ...
		  if (teacher.selected) $scope.albumNameArray.push(teacher.id);
		});
		if($scope.albumNameArray.length > 0){
				$scope.form.classTeacher=$scope.albumNameArray;
		}
		else{
			$scope.form.classTeacher="";
			return false;
		}	
  }
  $scope.checkSubToggle = function(){
	   $scope.albumNameArray = [];
		angular.forEach($scope.subjectlist, function(subject){
			
		  if (subject.selected) $scope.albumNameArray.push(subject.id);
		});
		if($scope.albumNameArray.length > 0){
				$scope.form.classSubjects=$scope.albumNameArray;
		}
		else{
			$scope.form.classSubjects	="";
			return false;
		}
  ...
  $scope.changeView = function(view){
	...
	if(view == "add")
	{
		...
		$scope.subjectlist = $scope.allsubjects;
        angular.forEach($scope.subjectlist, function (subject) {
...
schoex.controller('myclassesController', function(dataFactory,$rootScope,$scope) {
  $scope.classes = {};
  $scope.subjects = {};
  $scope.views = {};
  $scope.views.list = true;
  ...
  dataFactory.httpRequest('myclasses/listAll').then(function(data) {
    $scope.classes = data.classes;
	$scope.gradelevels =data.gradelevels;
    showHideLoad(true);
  });

 
  $scope.viewSubjects = function(id){
      showHideLoad();
      dataFactory.httpRequest('myclasses/subjectslist/'+id).then(function(data) {
        $scope.subjectslist = data.subjects;
		$scope.classid= data.classid;
        $scope.changeView('subjects');
        showHideLoad(true);
      });
  }
  
  $scope.viewStudents = function(id,subid){
      showHideLoad();
      dataFactory.httpRequest('myclasses/studentslist/'+id+'/'+subid).then(function(data) {
        $scope.studentlist = data.students;
        $scope.changeView('students');
        showHideLoad(true);
      });
  }
  ...
  $scope.attendance = function(id,clid){
    showHideLoad();
  ...
  /*  Student related function end */
  
  
  
  
  /* Class Grade Setting function */
  
  $scope.viewGradeLevel = function(id){
      showHideLoad();
      dataFactory.httpRequest('classgrade/gradelist/'+id).then(function(data) {
        $scope.gradelist = data.gradelist;
		$scope.classdata=data.classes;
        $scope.changeView('gradelevel');
		$scope.classid = id;
        showHideLoad(true);
      });
  }
  $scope.saveAdd = function(clsid){
    showHideLoad();
    dataFactory.httpRequest('classgrade/addgrade/'+clsid,'POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'add');
      if(data.status == "success"){
        $scope.gradelist.push(response);
        $scope.changeView('gradelevel');
      }
      showHideLoad(true);
    });
  ...
  $scope.edit = function(id){
    showHideLoad();
    dataFactory.httpRequest('classgrade/editgrade/'+id).then(function(data) {
      $scope.form = data;
      $scope.changeView('edit');
      showHideLoad(true);
    });
  ...
  $scope.saveEdit = function(){
    showHideLoad();
    dataFactory.httpRequest('classgrade/editgrade/'+$scope.form.id,'POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'edit');
      if(data.status == "success"){
        $scope.gradelist = apiModifyTable($scope.gradelist,response.id,response);
        $scope.changeView('gradelevel');
      }
      showHideLoad(true);
    });
  ...
  $scope.remove = function(item,index){
    ...
    if (confirmRemove == true) {
      showHideLoad();
      dataFactory.httpRequest('classgrade/delete/'+item.id,'POST').then(function(data) {
        response = apiResponse(data,'remove');
        if(data.status == "success"){
  ...
  /* Class Grade Setting End */
  
  /* overrule grade */
  
  $scope.viewOvrSubjects = function(id){
      showHideLoad();
      dataFactory.httpRequest('overrulegrade/subjectslist/'+id).then(function(data) {
        $scope.subjectslist = data.subjects;
		$scope.classid=data.classid;
		$scope.className = data.className;
		
        $scope.changeView('ovrsubjects');
        showHideLoad(true);
      });
  }
  
  $scope.viewStudentslist = function(id){
      showHideLoad();
      dataFactory.httpRequest('dashboard/classStudentlist/'+id).then(function(data) {
        $scope.classStudentlist = data.studentList;
		$scope.clid = id;
		$scope.changeView('viewStudentslist');
        showHideLoad(true);
      });
  }
  
  $scope.viewOvrStudents = function(clsid,subid){
      showHideLoad();
      dataFactory.httpRequest('overrulegrade/studentslist/'+clsid+'/'+subid).then(function(data) {
        $scope.studentlist = data.students;
		$scope.gradelevels = data.gradelevels;
        $scope.changeView('ovrstudents');
  ...
  $scope.ovraddgrade = function(uid,clid,subid){
      showHideLoad();
      dataFactory.httpRequest('overrulegrade/studentslist/'+$scope.classid+'/'+$scope.subjectid).then(function(data) {
        $scope.gradelevels = data.gradelevels;
		$scope.userid = uid;
		$scope.clid = clid;
		$scope.subid = subid;
        $scope.changeView('ovraddgrade');
        showHideLoad(true);
      });
  }
  
  $scope.ovreditgrade = function(ovrid){
      showHideLoad();
      dataFactory.httpRequest('overrulegrade/editgrade/'+ovrid).then(function(data) {
		$scope.form = data;
		$scope.gradelevels = data.gradelevels;
        $scope.changeView('ovreditgrade');
        showHideLoad(true);
      });
  }
  
  $scope.saveAddGrade = function(){
    showHideLoad();
    dataFactory.httpRequest('overrulegrade/addgrade/'+$scope.classid+'/'+$scope.subjectid+'/'+$scope.userid,'POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'add');
      if(data.status == "success"){
  ...
  $scope.saveEditGrade = function(){
    showHideLoad();
    dataFactory.httpRequest('overrulegrade/editgrade/'+$scope.form.id,'POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'add');
      if(data.status == "success"){
		$scope.gradelevels = apiModifyTable($scope.gradelevels,response.id,response);
        $scope.changeView('list');
  ...
  $scope.ovrremove = function(item,index){
    var confirmRemove = confirm($rootScope.phrase.sureRemove);
    if (confirmRemove == true) {
      showHideLoad();
      dataFactory.httpRequest('overrulegrade/delete/'+item.ovrid,'POST').then(function(data) {
        response = apiResponse(data,'remove');
        if(data.status == "success"){
  ...
  $scope.changeView = function(view){
    ...
    $scope.views.list = false;
    $scope.views.subjects = false;
    $scope.views.students = false;
	$scope.views.gradelevel = false;
	$scope.views.add = false;
	$scope.views.edit = false;
	$scope.views.ovrsubjects = false;
    $scope.views.ovrstudents = false;
	$scope.views.ovraddgrade = false;
	$scope.views.ovreditgrade = false;
	$scope.views.classstudents = false;
	$scope.views.attendance = false;
	$scope.views.marksheet = false;
	$scope.views.viewStudentslist = false;
	
    $scope.views[view] = true;
  }
});

schoex.controller('ClassesGradeController', function(dataFactory,$rootScope,$scope) {
  $scope.classes = {};
  $scope.classid ={};
  $scope.gradelevels = {};
  ...
  $scope.views.list = true;
  $scope.classlist = {};
  dataFactory.httpRequest('classgrade/listAll').then(function(data) {
    $scope.classes = data.classes;
	$scope.gradelevels =data.gradelevels;
  ...
  $scope.viewStudents = function(id,subid){
      showHideLoad();
      dataFactory.httpRequest('classgrade/studentslist/'+id+'/'+subid).then(function(data) {
        $scope.studentlist = data.students;
        $scope.changeView('students');
        showHideLoad(true);
      });
...
schoex.controller('yearLevelController', function(dataFactory,$rootScope,$scope) {
  $scope.views = {};
  $scope.views.list = true;
  $scope.classlist = {};
  dataFactory.httpRequest('yearLevel/listAll').then(function(data) {
    $scope.yearlevels = data.yearlevels;
	showHideLoad(true);
  });

  $scope.saveAdd = function(clsid){
    showHideLoad();
    dataFactory.httpRequest('yearLevel/add','POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'add');
      if(data.status == "success"){
        $scope.yearlevels.push(response);
        $scope.changeView('list');
      }
      showHideLoad(true);
    });
  ...
  $scope.edit = function(id){
    showHideLoad();
    dataFactory.httpRequest('yearLevel/edit/'+id).then(function(data) {
      $scope.form = data;
      $scope.changeView('edit');
      showHideLoad(true);
    });
  ...
  $scope.saveEdit = function(){
    showHideLoad();
    dataFactory.httpRequest('yearLevel/edit/'+$scope.form.id,'POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'edit');
      if(data.status == "success"){
        $scope.yearlevels = apiModifyTable($scope.yearlevels,response.id,response);
        $scope.changeView('list');
      }
      showHideLoad(true);
    });
  ...
  $scope.remove = function(item,index){
    ...
    if (confirmRemove == true) {
      showHideLoad();
      dataFactory.httpRequest('yearLevel/delete/'+item.id,'POST').then(function(data) {
        response = apiResponse(data,'remove');
        if(data.status == "success"){
          $scope.yearlevels.splice(index,1);
        }
        showHideLoad(true);
      });
    }
...
schoex.controller('OverruleGradeController', function(dataFactory,$rootScope,$scope) {
	$scope.classes = {};
	$scope.subjects = {};
	$scope.gradelevels ={};
	$scope.views = {};
	...
	$scope.userid = {};
	$scope.views.list = true;
	dataFactory.httpRequest('overrulegrade/listAll').then(function(data) {
	$scope.classes = data.classes;
	showHideLoad(true);
  });
  ...
  $scope.viewStudents = function(clsid,subid){
      showHideLoad();
      dataFactory.httpRequest('overrulegrade/studentslist/'+clsid+'/'+subid).then(function(data) {
        $scope.studentlist = data.students;
		$scope.gradelevels = data.gradelevels;
  ...
  $scope.addgrade = function(uid){
      showHideLoad();
      dataFactory.httpRequest('overrulegrade/studentslist/'+$scope.classid+'/'+$scope.subjectid).then(function(data) {
        $scope.gradelevels = data.gradelevels;
		$scope.userid = uid;
        $scope.changeView('addgrade');
        showHideLoad(true);
      });
  }
  
  $scope.editgrade = function(ovrid){
      showHideLoad();
      dataFactory.httpRequest('overrulegrade/editgrade/'+ovrid).then(function(data) {
		$scope.form = data;
		$scope.gradelevels = data.gradelevels;
        $scope.changeView('editgrade');
        showHideLoad(true);
      });
  }
  ...
  $scope.changeView = function(view){
    ...
    $scope.views.subjects = false;
    $scope.views.students = false;
	$scope.views.addgrade = false;
	$scope.views.editgrade = false;
    $scope.views[view] = true;
  }
});
...
schoex.controller('subjectsController', function(dataFactory,$rootScope,$scope) {
  ...
  $scope.checkToggle = function(){
	   $scope.albumNameArray = [];
		...
		if($scope.albumNameArray.length > 0){
				$scope.form.teacherId=$scope.albumNameArray;
		}
		else{
			$scope.form.teacherId="";
			return false;
		}
  ...
  $scope.edit = function(id){
    ...
    dataFactory.httpRequest('subjects/'+id).then(function(data) {
      $scope.form = data;
	  $scope.teachers=data.teachers;
	  $scope.form.teacherId=data.teacherId;
      $scope.changeView('edit');
      showHideLoad(true);
    });
  }
...
schoex.controller('reportsController', function(dataFactory,$rootScope,$scope,$http) {
  $scope.views = {};
  $scope.form = {};
  $scope.views.list = true;
  $scope.stats = {};
...
schoex.controller('assignmentsController', function(dataFactory,$rootScope,$sce,CSRF_TOKEN,$scope) {
  $scope.classes = {};
  $scope.subject = {};
  $scope.assignments = {};
  $scope.views = {};
  ...
  dataFactory.httpRequest('assignments/listAll').then(function(data) {
	...
	$scope.yearlevels = data.yearlevels;
	$scope.gradelevels = data.gradelevels;
	$scope.allsubject = data.allsubjects;
    $scope.userRole = data.userRole;
    showHideLoad(true);
	response = apiResponse(data,'add');
  });
  ...
  $scope.assignmentClasses = function(id){
      showHideLoad();
      dataFactory.httpRequest('assignments/assignmentClasses/'+id).then(function(data) {
          $scope.assgnClasses = data.classes;
		  $scope.assignmentId = data.assignmentInfo.id;
		  $scope.assignmentInfo = data.assignmentInfo;
          $scope.changeView('assignmentClasses');
          showHideLoad(true);
      });
  }
  
  $scope.viewSchedule = function(asid,clsid){
      showHideLoad();
      dataFactory.httpRequest('assignments/viewSchedule/'+asid+'/'+clsid).then(function(data) {
          $scope.scheduleData = data.studentsSchedule;
		  $scope.assignmentInfo = data.assignmentInfo;
		  $scope.classInfo = data.classInfo;
          $scope.changeView('viewSchedule');
          showHideLoad(true);
      });
  }
  ...
  $scope.checkToggle = function(){
	   $scope.albumNameArray = [];
		angular.forEach($scope.classes, function(classdata){
		  if (classdata.selected) $scope.albumNameArray.push(classdata.id);
		});
		if($scope.albumNameArray.length > 0){
				$scope.form.classId=$scope.albumNameArray;
		}
		else{
			$scope.form.classId="";
			return false;
	...
	$scope.savetodraft = "";
	  $scope.saveAssgndraft = function(asid){
			dataFactory.httpRequest('assignments/saveAssgndraft/'+asid,'POST',{},$scope.form).then(function(data) {
			if(data.savedraft== "save")
			{
				$scope.savetodraft = "";
				$scope.savetodraft = "saved";
			}	
		});
	  }
  $scope.subjectList = function(){
      dataFactory.httpRequest('dashboard/allsubjectList','POST',{},{"classes":$scope.form.classId}).then(function(data) {
          $scope.subject = data;
          $scope.form.subject = data;
      });
  }

  $scope.levelpathlevel = function(){
      dataFactory.httpRequest('dashboard/levelpathlevel','POST',{},{"classes":$scope.form.classId}).then(function(data) {
          $scope.gradelevels = data;
      });
  }
  ...
  $scope.edit = function(id){
    ...
    dataFactory.httpRequest('assignments/'+id).then(function(data) {
      ...
      $scope.form = data;
	  $scope.classes = data.classes;
	  $scope.subject = data.subjectlist;
      showHideLoad(true);
    });
  }
  
   $scope.viewAssignment = function(id){
    showHideLoad();
    dataFactory.httpRequest('assignments/viewAssignment/'+id).then(function(data) {
      $scope.changeView('viewAssignment');
      $scope.assignmentData = data;
	  $scope.form.AssignDescription = data.draftassignass;
	  $scope.assignmentContent = $sce.trustAsHtml(data.AssignDescription);
      showHideLoad(true);
    });
  }
  
  $scope.viewStudentAnwser = function(id){
    showHideLoad();
    dataFactory.httpRequest('assignments/viewStudentAnwser/'+id).then(function(data) {
      $scope.changeView('viewStudentAnwser');
      $scope.assignmentData = data;
	  $scope.assignmentReply = $sce.trustAsHtml(data.userNotes);
      showHideLoad(true);
    });
  }
  ...
  $scope.giveGrade = function(id){
      showHideLoad();
      dataFactory.httpRequest('assignments/giveGrade/'+id).then(function(data) {	
          $scope.form = data;
          $scope.changeView('giveGrade');
          showHideLoad(true);
      });
  }
  ...
  $scope.saveGrade = function(){
    ...
    dataFactory.httpRequest('assignments/giveGrade/'+$scope.form.id,'POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'edit');
      if(data.status == "success"){
        $scope.assignments = apiModifyTable($scope.assignments,response.id,response);
        $scope.changeView('list');
  ...
  $scope.giveFeedback = function(id){
      showHideLoad();
      dataFactory.httpRequest('assignments/giveFeedback/'+id).then(function(data) {	
          $scope.form = data;
          $scope.changeView('giveFeedback');
          showHideLoad(true);
      });
  }
  ...
  $scope.saveFeedback = function(){
    ...
    dataFactory.httpRequest('assignments/giveFeedback/'+$scope.form.id,'POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'edit');
      if(data.status == "success"){
        $scope.assignments = apiModifyTable($scope.assignments,response.id,response);
        $scope.changeView('list');
  ...
  $scope.viewGradeFeedback = function(id){
      showHideLoad();
      dataFactory.httpRequest('assignments/viewGradeFeedback/'+id).then(function(data) {
          $scope.gradeFeedback = data.assignments;
          $scope.changeView('viewGradeFeedback');
          showHideLoad(true);
      });
  }
  $scope.remove = function(item,index){
  ...
  $scope.changeView = function(view){
    if(view == "add" || view == "list" || view == "show"){
      $scope.form = {};
	  $scope.form.AssignDescription="";
    }
	if(view == "add")
	{
		$scope.classes = $scope.allclasses;
        angular.forEach($scope.classes, function (classes) {
	...
	$scope.views.giveGrade = false;
	$scope.views.giveFeedback = false;
	
	$scope.views.viewGradeFeedback = false;
	$scope.views.viewAssignment = false;
	$scope.views.viewStudentAnwser = false;
	$scope.views.assignmentClasses = false;
	$scope.views.viewSchedule = false;
...
schoex.controller('ScheduleAssignmentsController', function(dataFactory,$rootScope,$sce,CSRF_TOKEN,$scope) {
  $scope.classes = {};
  $scope.subject = {};
  $scope.assignments = {};
  $scope.views = {};
  ...
  dataFactory.httpRequest('scheduleAssignments/listAll').then(function(data) {
    $scope.assignments = data.assignments;
    showHideLoad(true);
  });
  
  
  $scope.setschedule = function(id){
    showHideLoad();
    dataFactory.httpRequest('scheduleAssignments/getAssignment/'+id).then(function(data) {
      $scope.changeView('setSchedule');
      $scope.form = data;
      showHideLoad(true);
    });
  }
  ...
  $scope.saveSchedule = function(id){
    showHideLoad();
    console.log($scope.form);
    dataFactory.httpRequest('scheduleAssignments/setSchedule/'+id,'POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'edit');
      if(data.status == "success"){
			$scope.assignments = apiModifyTable($scope.assignments,response.id,response);  
			$scope.changeView('list');
   ...
   $scope.viewAssignment = function(id){
    showHideLoad();
    dataFactory.httpRequest('scheduleAssignments/viewAssignments/').then(function(data) {
      $scope.changeView('viewAssignment');
      $scope.assignmentData = data;
	  $scope.assignmentCassignmentContent = $sce.trustAsHtml(data.AssignDescription);
      showHideLoad(true);
    });
  }
  
  $scope.removeSchedule = function(item,index){
    var confirmRemove = confirm($rootScope.phrase.sureRemove);
    if (confirmRemove == true) {
      showHideLoad();
      dataFactory.httpRequest('scheduleAssignments/removeSchedule/'+item.id,'POST').then(function(data) {
        response = apiResponse(data,'remove');
        if(data.status == "success"){
          $scope.assignments.splice(index,1);
        }
  ...
  $scope.changeView = function(view){
    ...
    $scope.views.list = false;
	$scope.views.setSchedule = false;
	
    $scope.views[view] = true;
  }
});

schoex.controller('ScheduleOnlineExamsController', function(dataFactory,$rootScope,$sce,CSRF_TOKEN,$scope) {
  $scope.classes = {};
  $scope.subject = {};
  $scope.assignments = {};
  $scope.views = {};
  ...
  dataFactory.httpRequest('scheduleOnlineExams/listAll').then(function(data) {
  ...
  $scope.setschedule = function(id){
    showHideLoad();
    dataFactory.httpRequest('scheduleOnlineExams/getOnlineexams/'+id).then(function(data) {
      $scope.changeView('setSchedule');
      $scope.form = data;
      showHideLoad(true);
    });
  ...
  $scope.saveSchedule = function(id){
    showHideLoad();
    console.log($scope.form);
    dataFactory.httpRequest('scheduleOnlineExams/setSchedule/'+id,'POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'edit');
      if(data.status == "success"){
  ...
  $scope.removeSchedule = function(item,index){
    ...
    if (confirmRemove == true) {
      showHideLoad();
      dataFactory.httpRequest('scheduleOnlineExams/removeSchedule/'+item.id,'POST').then(function(data) {
        response = apiResponse(data,'remove');
        if(data.status == "success"){
...
schoex.controller('onlineExamsController', function(dataFactory,$rootScope,$scope,$sce) {
  ...
  $scope.onlineexamClasses = function(id){
      showHideLoad();
      dataFactory.httpRequest('onlineExams/onlineexamClasses/'+id).then(function(data) {
          $scope.onexamsClasses = data.classes;
		  $scope.onlineexamid = data.onlineexamInfo.id;
		  $scope.onlineexamInfo = data.onlineexamInfo;
          $scope.changeView('onlineexamClasses');
          showHideLoad(true);
      });
  }
  
  $scope.viewSchedule = function(onid,clsid){
      showHideLoad();
      dataFactory.httpRequest('onlineExams/viewSchedule/'+onid+'/'+clsid).then(function(data) {
          $scope.scheduleData = data.studentsSchedule;
		  $scope.onlineexamInfo = data.onlineexamInfo;
		  $scope.classInfo = data.classInfo;
          $scope.changeView('viewSchedule');
  ...
  $scope.checkToggle = function(){
	   $scope.albumNameArray = [];
		...
		if($scope.albumNameArray.length > 0){
				$scope.form.examClass=$scope.albumNameArray;
		}
		else{
			$scope.form.examClass="";
			return false;
		}
  }
  ...
  $scope.levelpathlevel = function(){
      dataFactory.httpRequest('dashboard/levelpathlevel','POST',{},{"classes":$scope.form.examClass}).then(function(data) {
          $scope.gradelevels = data;
      });
  }
  ...
  $scope.removeQuestion = function(question,index){
	...
	// $(".examsquest").removeClass("activequest");
    // $("#"+block+"examsquestion-"+index).addClass("activequest");
    // $(".questlist li").removeClass("activefirst");
    // $("."+block+"qst #"+index).addClass("activefirst");
	
	$(".examsquest").removeClass("activequest");
    $("#addexamsquestion-"+index).addClass("activequest");
    $(".questlist li").removeClass("activefirst");
    $(".addqst #"+index).addClass("activefirst");
	
	$(".examsquest").removeClass("activequest");
    $("#editexamsquestion-"+index).addClass("activequest");
    $(".questlist li").removeClass("activefirst");
    $(".editqst #"+index).addClass("activefirst");
  ...
  $scope.changeView = function(view){
    ...
    $scope.views.marks = false;
	$scope.views.onlineexamClasses = false;
    $scope.views.viewSchedule = false;
    $scope.views[view] = true;
  }
});
schoex.controller('learnPathController', function(dataFactory,$rootScope,$scope,$location,$sce) {
  $scope.classes = {};
  $scope.allclasses = {};
  $scope.subject = {};
  $scope.subjectsname = {};
  ...
  $scope.ConvertToHTML = function(assignment) {
	  if(assignment.AssignDescription != "")
	  {
		return $sce.trustAsHtml(assignment.AssignDescription);
	  }
  }; 
  
	
  dataFactory.httpRequest('learnPath/listAll').then(function(data) {	
    $scope.classes = data.classes;
	$scope.allclasses = data.classes;
	$scope.learnpaths = data.learnPaths;
	...
	$scope.gradelevels = data.gradelevels;
	$scope.yearlevels = data.yearlevels;
	//$scope.requirements = [{ name: "Mandatory", id: 1 }, { name: "Optional", id: 2 }];
	$scope.status = [{ name: "Active", id: 1 }, { name: "Deactive", id: 0 }];
    showHideLoad(true);
  });


  /*  learnpath drag drop start */
  $scope.lpmoveblockdown = function(id,block){
		var pid = $("#"+block+"-"+id+"-td").parent().attr("id");
		var nxtpid =  $("tr#"+pid).next().attr('id');
		var cur_new_sortid = $("#"+pid+" .sortorder").val();
		var next_new_sortid = $("#"+nxtpid+" .sortorder").val();

		var currenthtml = $("tr#"+pid).html();
		var nexthtml = $("tr#"+nxtpid).html();
		
		var mid = [];
		mid[0] = $("tr#"+pid+" .mid").val();
		mid[1] = $("tr#"+nxtpid+" .mid").val();
		
		$("#"+pid).fadeOut();
		$("tr#"+pid).html(nexthtml);
		$("#"+pid).fadeIn();
		
		$("#"+nxtpid).fadeOut();
		$("tr#"+nxtpid).html(currenthtml);
		$("#"+nxtpid).fadeIn();
		
		
		$("tr#"+pid+" .sortorder").val(cur_new_sortid);
		$("tr#"+nxtpid+" .sortorder").val(next_new_sortid);
			for(var i=0;i < $scope.form.moduleList.length;i++){
				if($scope.form.moduleList[i].mId == mid[0]){
					$scope.form.moduleList[i].morder =next_new_sortid ;
				}
				
				if($scope.form.moduleList[i].mId == mid[1]){
					$scope.form.moduleList[i].morder =cur_new_sortid ;
				}
			}			
	}
	
	$scope.lpmoveblockup = function(id,block){
		var pid = $("#"+block+"-"+id+"-td").parent().attr("id");
		var prevpid =  $("tr#"+pid).prev().attr('id');
		var cur_new_sortid = $("#"+pid+" .sortorder").val();
		var prev_new_sortid = $("#"+prevpid+" .sortorder").val();
		
		var currenthtml = $("tr#"+pid).html();
		var nexthtml = $("tr#"+prevpid).html();
		
		var mid = [];
		mid[0] = $("tr#"+pid+" .mid").val();
		mid[1] = $("tr#"+prevpid+" .mid").val();
		...
		$("#"+prevpid).fadeOut();
		$("tr#"+prevpid).html(currenthtml);
		$("#"+prevpid).fadeIn();
		
		
		$("tr#"+pid+" .sortorder").val(cur_new_sortid);
		$("tr#"+prevpid+" .sortorder").val(prev_new_sortid);
			for(var i=0;i < $scope.form.moduleList.length;i++){
				if($scope.form.moduleList[i].mId == mid[0]){
					$scope.form.moduleList[i].morder =prev_new_sortid ;
				}
  ...
  $scope.removeModule = function(mid){
	for(var i=0;i < $scope.form.moduleList.length;i++){
		if($scope.form.moduleList[i].mId == mid){
			$scope.form.moduleList.splice(i,1);
			
		}
	}
  }
  
  // $scope.removeModule = function(item,index){
    // $scope.form.moduleList.splice(index,1);
    // $scope.form.moduleids.splice(index,1);
   // }
   
  /*  learnpath drag drop end */

	$scope.textdraft = function(id){
      dataFactory.httpRequest('assignments/saveAssgndraft/'+id,'POST',{},{"textareadata":$scope.form.AssignDescription}).then(function(data) {
        if(data.savedraft== "save")
		{
			$scope.savetodraft = "saved";
		}	
	...
	$scope.checkToggle = function(){
	   $scope.albumNameArray = [];
		...
		if($scope.albumNameArray.length > 0){
				$scope.form.lpClasses=$scope.albumNameArray;
		}
		else{
			$scope.form.lpClasses="";
			return false;
		}
    }
	$scope.closeModule = function(){
		$("#addnewmodule").fadeOut();
	}
	$scope.addModule = function(){
		$("#addnewmodule").fadeIn();
		$scope.form.addmodgradelevel = $scope.form.gradelevel;
		$scope.form.addmodylevel = $scope.form.yearlevel;
		$scope.form.addmodSubject = $scope.form.subject;
		$scope.form.modClasses = $scope.form.lpClasses;
	}
	
	$scope.subjectList = function(){
		dataFactory.httpRequest('dashboard/allsubjectList','POST',{},{"classes":$scope.form.modClasses}).then(function(data) {
			$scope.subject = data;
		});
	}
  
	  $scope.levelpathlevel = function(){
		  dataFactory.httpRequest('dashboard/levelpathlevel','POST',{},{"classes":$scope.form.modClasses}).then(function(data) {
			  $scope.gradelevels = data;
		  });
	  }
	...
	$scope.addnewModule = function(){
		showHideLoad();
		dataFactory.httpRequest('dashboard/addnewmodule','POST',{},$scope.form).then(function(data) {
			if(data.response == "added")
			{
				$scope.modules = data.modules;
				$scope.moduleadded = data.moduleadded;
				$("#addnewmodule").fadeOut();
				var moduleData = {};
				moduleData.mId = data.moduleadded.id;
				//moduleData.mstatus = $scope.form.status;
				moduleData.morder = $scope.form.moduleList.length;
				moduleData.mtitle = data.moduleadded.moduleTitle;
				
				$scope.form.moduleList.push(moduleData);
				console.log($scope.form.moduleList);
				
				var moduleIdData = {};
				$scope.form.moduleids = [];
				moduleIdData.mId = data.moduleadded.id;
				$scope.form.moduleids.push(moduleIdData);
				console.log($scope.form.moduleids);
	
			}
			else
			{
				alert("Problem in adding data");
				return false;
			}
			showHideLoad(true);
		});
	}
	$scope.addModuleData = function(){
    if (typeof $scope.form.modules === "undefined" || $scope.form.modules == "") {
      alert("Select Module");
      return ;
    }
	
	// if (typeof $scope.form.sortorder === "undefined" || $scope.form.sortorder == "") {
      // alert("Add Sort Order");
      // return ;
    // }

	var moduleData = {};
    moduleData.mId = $scope.form.modules;
    //moduleData.mstatus = $scope.form.status;
	moduleData.morder = $scope.form.moduleList.length;
	moduleData.mtitle = $.grep($scope.modules, function (mod) {
		return mod.id == moduleData.mId;
	})[0].moduleTitle;
    $scope.form.moduleList.push(moduleData);
    console.log($scope.form.moduleList);
	...
	var moduleIdData = {};
	$scope.form.moduleids = [];
	moduleIdData.mId = $scope.form.modules;
	$scope.form.moduleids.push(moduleIdData);
    console.log($scope.form.moduleids);
   ...
   $scope.editModuleSort = function(item,index){
	   var value =$("#order-"+item.mId).val();
       item.morder = value;	
   }
   
   $scope.changeModuleSort = function(item,index){
	   var value =$("#changeorder-"+item.mId).val();
       item.morder = value;	
   }
		
	 $scope.moduleEdit = function(id){
		showHideLoad();
		var lpid = $scope.form.id;

		dataFactory.httpRequest('modules/'+id).then(function(data) {
		  //$location.path('/modules/');
		  $scope.changeView('moduleEdit');
		  $scope.form = data;
		  $scope.lpid = lpid;
		  $scope.modules = data.modules;
		  $scope.assignments = data.assignments;
	...
	$scope.savemoduleEdit = function(){
    showHideLoad();
    console.log($scope.form);
	
    dataFactory.httpRequest('modules/'+$scope.form.id,'POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'edit');
      if(data.status == "success"){
        $scope.modules = apiModifyTable($scope.modules,response.id,response);
        $scope.changeView('list');
      }
      showHideLoad(true);
    });
  }
  /* Assignment And OnlineExam related Function */
	
	
	$scope.assignmentEdit = function(id){
		showHideLoad();
		var modid = $scope.form.id;
		
		dataFactory.httpRequest('assignments/'+id).then(function(data) {
		  $scope.changeView('editAssignment');
		  $scope.gradelevels = data.gradelevels;
		  $scope.form = data;
		  $scope.modid = modid;
		  $scope.classes = data.classes;
    ...
    $scope.saveEditAssignment = function(content,section){
		response = apiResponse(content,'edit');
		if(content.status == "success"){
		  showHideLoad();

		  $scope.assignments = apiModifyTable($scope.assignments,response.id,response);
		  if(section == "addmodule")
		  {
			  $scope.changeView('add');
		  }
		  $scope.changeView('list');
	...
	$scope.editOnlineExam = function(id){
    showHideLoad();
	var modid = $scope.form.id;
    dataFactory.httpRequest('onlineExams/'+id).then(function(data) {
      $scope.changeView('editOnlineExam');
	  $scope.gradelevels = data.gradelevels;
      $scope.form = data;
	  $scope.modid = modid;
	  $scope.classes = data.classes;
  ...
  $scope.saveEditOnlineExam = function(){
    showHideLoad();
    console.log($scope.form);
    dataFactory.httpRequest('onlineExams/'+$scope.form.id,'POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'edit');
  ...
  $scope.removeQuestion = function(question,index){
	  ...
	  // console.log(question);
	   // if(index){
		   // start = start+1;
		   // return $scope.form.examQuestion.splice(start,1);
	   // alert("question is"+index);
	   // }
	   
		$scope.form.examQuestion.splice(index,1);
		//$scope.form.examQuestion.splice(question,1);
		//alert(examQuestion);
  ...
  /* Modules relates function */
  $scope.Mlevelpathlevel = function(){
      dataFactory.httpRequest('dashboard/levelpathlevel','POST',{},{"classes":$scope.form.mdClasses}).then(function(data) {
          $scope.gradelevels = data;
      });
  }
  
  $scope.MsubjectList = function(){
		dataFactory.httpRequest('dashboard/allsubjectList','POST',{},{"classes":$scope.form.mdClasses}).then(function(data) {
			$scope.subject = data;
		});
  }
  
	$scope.selectAssgnOnexm = function(){
	dataFactory.httpRequest('dashboard/selectAssgnOnexm','POST',{},{"classes":$scope.form.mdClasses,"subject":$scope.form.subject,"level":$scope.form.gradelevel,"yearlevel":$scope.form.yearlevel}).then(function(data) {
		 $scope.assignments = data.assignments;
		 $scope.onlineexams = data.onlineExams;
	});
}
	...
	$scope.adasgcheckToggle = function(){
	   $scope.albumNameArray = [];
		angular.forEach($scope.classes, function(classdata){
		  if (classdata.selected) $scope.albumNameArray.push(classdata.id);
		});
	...
	$scope.addAssign = function(){
		$(".addnewmodule").fadeOut();
		$("#addnewassign").fadeIn();
		$scope.form.level = $scope.form.gradelevel;
		$scope.form.ylevel = $scope.form.yearlevel;
		$scope.form.assubjectId = $scope.form.subject;
		$scope.form.classId = $scope.form.mdClasses;
		
	}
	$scope.addNewExams = function(){
		$(".addnewmodule").fadeOut();
		$("#addnewexam").fadeIn();
		$scope.form.addonexgradelevel = $scope.form.gradelevel;
		$scope.form.addonexylevel = $scope.form.yearlevel;
		$scope.form.addonexexamSubject = $scope.form.subject;
		$scope.form.classId = $scope.form.mdClasses;
	}
	$scope.addnewAssign = function(){
		showHideLoad();
		dataFactory.httpRequest('dashboard/addnewassign','POST',{},$scope.form).then(function(data) {
			if(data.response == "added")
			{
				$scope.assignments = data.assignments;
				$scope.assignmentadded = data.assignmentadded;
				$(".addnewmodule").fadeOut();
				var assignmentData = {};
				assignmentData.assId = data.assignmentadded.id;
				assignmentData.assreq ="1";
				assignmentData.assorder =$scope.form.modulesetlist.length;
				assignmentData.asstitle = data.assignmentadded.AssignTitle;
				$scope.form.assignmentsList.push(assignmentData);
				console.log($scope.form.assignmentsList);
				assignmentData.mixid =$scope.form.assignment;
				assignmentData.sortorder =$scope.form.sortorder;
				assignmentData.setid ="assid";
				$scope.form.modulesetlist.push(assignmentData);
			}
			else
			{
				alert("Problem in adding data");
	...
	$scope.addnewExam = function(){
		showHideLoad();
		dataFactory.httpRequest('dashboard/addnewexam','POST',{},$scope.form).then(function(data) {
			if(data.response == "added")
			{
				$scope.onlineexams = data.onlineExams;
				$scope.onexmadded = data.onexmadded;
				$(".addnewmodule").fadeOut();
				var onExData = {};
				onExData.oexId = data.onexmadded.id;
				onExData.oexreq = "1";
				onExData.oexorder = $scope.form.modulesetlist.length;
				onExData.oextitle = data.onexmadded.examTitle
				$scope.form.onlineexamsList.push(onExData);
				console.log($scope.form.onlineexamsList);
				onExData.mixid =$scope.form.onlineExams;
				onExData.sortorder =$scope.form.oexsortorder;
				onExData.setid ="oexid";
				$scope.form.modulesetlist.push(onExData);
	...
	$scope.asexsubjectList = function(responseFrom){
		if(responseFrom == "assignment")
		{
			dataFactory.httpRequest('dashboard/allsubjectList','POST',{},{"classes":$scope.form.classId}).then(function(data) {
				$scope.subject = data;
			});
	...
	$scope.subjectList = function(){
      dataFactory.httpRequest('dashboard/subjectList','POST',{},{"classes":$scope.form.lpClasses}).then(function(data) {
          $scope.subject = data;
  ...
  /* module drag drop start */
  
  $scope.moveblockdown = function(id,movefrom,mixid,block){
		var pid = $("#"+block+"-"+id+"-td").parent().attr("id");
		var nxtpid =  $("#"+pid).next().attr('id');
		var cur_new_sortid = $("#"+pid+" .sortorder").val();
		var next_new_sortid = $("#"+nxtpid+" .sortorder").val();
		
		var mixArr = [];
		var sf = [];
		mixArr[0] = $("tr#"+pid+" .inputtypeid").val();
		sf[0] = $("tr#"+pid+" .inputtypesf").val();
		mixArr[1] = $("tr#"+nxtpid+" .inputtypeid").val();
		sf[1] = $("tr#"+nxtpid+" .inputtypesf").val();
		...
		$("tr#"+nxtpid+" .sortorder").val(next_new_sortid);
			if(movefrom == "assid"){
				if(sf[1] == "assid"){
					for(var i=0;i < $scope.form.assignmentsList.length;i++){
					
						if($scope.form.assignmentsList[i].assId == mixArr[0]){
							$scope.form.assignmentsList[i].assorder = next_new_sortid;
						}
						if($scope.form.assignmentsList[i].assId == mixArr[1]){
							$scope.form.assignmentsList[i].assorder = cur_new_sortid;
						}
					}
				}
				else{
					if(sf[1] == "oexid"){
						...
						for(var i=0;i < $scope.form.onlineexamsList.length;i++){
								if($scope.form.onlineexamsList[i].oexId == mixArr[1]){
									$scope.form.onlineexamsList[i].oexorder = cur_new_sortid;
								}
						}
					}
				}
			}
			else if(movefrom == "oexid"){
				if(sf[1] == "oexid"){
					for(var i=0;i < $scope.form.onlineexamsList.length;i++){
						if($scope.form.onlineexamsList[i].oexId == mixArr[0]){
							$scope.form.onlineexamsList[i].oexorder = next_new_sortid;
						}
						if($scope.form.onlineexamsList[i].oexId == mixArr[1]){
							$scope.form.onlineexamsList[i].oexorder = cur_new_sortid;
		...
		$scope.moveblockup = function(id,movefrom,mixid,block){
			var pid = $("td#"+block+"-"+id+"-td").parent().attr("id");
			var previd = $("#"+pid).prev().attr('id');
			var cur_new_sortid = $("tr#"+pid+" .sortorder").val();
			var prev_new_sortid = $("tr#"+previd+" .sortorder").val();
			
			var mixArr = [];
			var sf = [];
			mixArr[0] = $("tr#"+pid+" .inputtypeid").val();
			sf[0] = $("tr#"+pid+" .inputtypesf").val();
			mixArr[1] = $("tr#"+previd+" .inputtypeid").val();
			sf[1] = $("tr#"+previd+" .inputtypesf").val();
			
			
			var currenthtml = $("tr#"+pid).html();
			var prevhtml = $("tr#"+previd).html();
			$("#"+pid).fadeOut();
			$("tr#"+pid).html(prevhtml);
			$("#"+pid).fadeIn();

			$("#"+previd).fadeOut();
			$("tr#"+previd).html(currenthtml);
			$("#"+previd).fadeIn();
			
			$("tr#"+pid+" .sortorder").val(cur_new_sortid);
			$("tr#"+previd+" .sortorder").val(prev_new_sortid);
				if(movefrom == "assid"){
					if(sf[1] == "assid"){
						for(var i=0;i < $scope.form.assignmentsList.length;i++){
						
							if($scope.form.assignmentsList[i].assId == mixArr[0]){
								$scope.form.assignmentsList[i].assorder = prev_new_sortid;
							}
							if($scope.form.assignmentsList[i].assId == mixArr[1]){
								$scope.form.assignmentsList[i].assorder = cur_new_sortid;
							}
					...
					if(sf[1] == "oexid"){
						for(var i=0;i < $scope.form.onlineexamsList.length;i++){
							...
							if($scope.form.onlineexamsList[i].oexId == mixArr[1]){
								$scope.form.onlineexamsList[i].oexorder = prev_new_sortid;
  ...
  $scope.addAssignmentData = function(){
		if (typeof $scope.form.assignment === "undefined" || $scope.form.assignment == "") {
		  alert("Select Assignment");
		  return ;
		}

		if (typeof $scope.form.requirement === "undefined" || $scope.form.requirement == "") {
		  alert("Select requirements "+$scope.form.requirements);
		  return ;
		}
		// if (typeof $scope.form.sortorder === "undefined" || $scope.form.sortorder == "") {
		  // alert("Select Sort Order");
		  // return ;
		// }
		
		var assignmentData = {};
		assignmentData.assId = $scope.form.assignment;
		assignmentData.assreq = $scope.form.requirement;
		assignmentData.assorder = $scope.form.modulesetlist.length;
		// assignmentData.assorder = $scope.form.sortorder;
		
		assignmentData.asstitle = $.grep($scope.assignments, function (assgn) {
			return assgn.id == assignmentData.assId;
		})[0].AssignTitle;
		...
		$scope.form.modulesetlist.push(assignmentData);
		console.log($scope.form.modulesetlist);
	...
	$scope.removeAssignment = function(asid){
		for(var i=0;i < $scope.form.assignmentsList.length;i++){
			if($scope.form.assignmentsList[i].assId == asid){
				$scope.form.assignmentsList.splice(i,1);
			}
		}
	}
	  
	  $scope.removeOnlineExam = function(onexid){
		 for(var i=0;i < $scope.form.onlineexamsList.length;i++){
			if($scope.form.onlineexamsList[i].oexId == onexid){
				$scope.form.onlineexamsList.splice(i,1);
			}
		}
	  }
  
    $scope.addOnlineExamData = function(){
		if (typeof $scope.form.onlineExams === "undefined" || $scope.form.onlineExams == "") {
		  alert("Select Exam");
		  return ;
    }
	
	if (typeof $scope.form.oexrequirement === "undefined" || $scope.form.oexrequirement == "") {
      alert("Select requirements "+$scope.form.requirements);
      return ;
    }
	if (typeof $scope.form.oexsortorder === "undefined" || $scope.form.oexsortorder == "") {
      alert("Select Sort Order");
      return ;
    }

    var onExData = {};
    onExData.oexId = $scope.form.onlineExams;
    onExData.oexreq = $scope.form.oexrequirement;
	// onExData.oexorder = $scope.form.oexsortorder;
	onExData.oexorder =$scope.form.modulesetlist.length;
	
    onExData.oextitle = $.grep($scope.onlineexams, function (oex) {
		return oex.id == onExData.oexId;
	})[0].examTitle;
   ...
   $scope.editAssignSort = function(item,index,rowindex){
		   var value =$("#sortorder-"+rowindex).val();
		   //item.assorder = value;	
			for(var i=0;i < $scope.form.assignmentsList.length;i++){
			if($scope.form.assignmentsList[i].assId == item.assId){
				$scope.form.assignmentsList[i].assorder = value;
			 }
			}
	}
	$scope.editOnExSort = function(item,index,rowindex){
   
	   var value =$("#sortorder-"+rowindex).val();
	   //item.oexorder = value;	
		for(var i=0;i < $scope.form.onlineexamsList.length;i++){
		if($scope.form.onlineexamsList[i].oexId == item.oexId){
			$scope.form.onlineexamsList[i].oexorder = value;
		 }
		}
	   
   }
   /* module drag drop end */
   
   
   $scope.changeAssignSort = function(item,index){
	   var value =$("#changeorder-"+item.assId).val();
       item.assorder = value;	
   }
   
   // $scope.editOnExSort = function(item,index){
	   
	   // var value =$("#edoexorder-"+item.oexId).val();
       // item.oexorder = value;	
	   
   // }
   
    $scope.changeOnExSort = function(item,index){
	   var value =$("#changeorder-"+item.oexId).val();
       item.oexorder = value;	
   }
  
	  
	/* modules related function end */
	
	/* set schedule related function */
	$scope.learnpathid ="";
	$scope.scheduleItemList = function(lpid){
		showHideLoad();
		$scope.changeView('scheduleitemlist');	
		$scope.learnpathid = lpid;
		showHideLoad(true);
	}
	
	/*set assignment schedule */
	
	$scope.setAssgnschedule = function(lpid){
		showHideLoad();
		dataFactory.httpRequest('dashboard/getLpAssign/'+lpid).then(function(data) {
			//response = apiResponse(data,'remove');
			$scope.getLpAssign = data.assignments;
			$scope.changeView('scheduleAssgnlist');
			showHideLoad(true);
		  });
	}
	
	
	$scope.AssgnSchedule = function(id,lpid){
		showHideLoad();
		dataFactory.httpRequest('scheduleAssignments/getAssignment/'+id).then(function(data) {
		  $scope.changeView('AssgnSchedule');
		  $scope.form = data;
		  $scope.learnpathid = lpid
		  showHideLoad(true);
		});
	...
	$scope.saveAssgnSchedule = function(id){
		showHideLoad();
		console.log($scope.form);
		dataFactory.httpRequest('scheduleAssignments/setSchedule/'+id,'POST',{},$scope.form).then(function(data) {
		  response = apiResponse(data,'edit');
		  if(data.status == "success"){
				$scope.assignments = apiModifyTable($scope.assignments,response.id,response);  
				//$scope.changeView('list');
				$scope.viewLearnPath($scope.learnpathid);
		  }
		  showHideLoad(true);
		});
	}
  
  $scope.removeAssgnSchedule = function(item,index){
    var confirmRemove = confirm($rootScope.phrase.sureRemove);
    if (confirmRemove == true) {
      showHideLoad();
      dataFactory.httpRequest('scheduleAssignments/removeSchedule/'+item.id,'POST').then(function(data) {
        response = apiResponse(data,'remove');
        // if(data.status == "success"){
          // $scope.getLpAssign.splice(index,1);
        // }
        showHideLoad(true);
      });
    }
  ...
  /*set assignment schedule end */
	
  /*set onlineexam schedule */
	$scope.setOnexschedule = function(lpid){
		showHideLoad();
		dataFactory.httpRequest('dashboard/getLpOnExam/'+lpid).then(function(data) {
			//response = apiResponse(data,'remove');
			$scope.getLpOnexm = data.onlineexams;
			$scope.changeView('scheduleOnexmlist');
			showHideLoad(true);
		  });
	}
	$scope.OnExSchedule = function(id,lpid){
		showHideLoad();
		dataFactory.httpRequest('scheduleOnlineExams/getOnlineexams/'+id).then(function(data) {
		  $scope.changeView('OnlineexamSchedule');
		  $scope.form = data;
		  $scope.learnpathid = lpid;
		  showHideLoad(true);
		});
  ...
  $scope.saveOnexSchedule = function(id){
    showHideLoad();
    console.log($scope.form);
    dataFactory.httpRequest('scheduleOnlineExams/setSchedule/'+id,'POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'edit');
      if(data.status == "success"){
		$scope.getLpOnexm = apiModifyTable($scope.getLpOnexm,response.id,response);  
        $scope.viewLearnPath($scope.learnpathid);
      }
      showHideLoad(true);
    });
  ...
  $scope.removeOnexSchedule = function(item,index){
    var confirmRemove = confirm($rootScope.phrase.sureRemove);
    if (confirmRemove == true) {
      showHideLoad();
      dataFactory.httpRequest('scheduleOnlineExams/removeSchedule/'+item.id,'POST').then(function(data) {
        response = apiResponse(data,'remove');
        // if(data.status == "success"){
          // $scope.getLpOnexm.splice(index,1);
        // }
        showHideLoad(true);
      });
    }
  ...
  /*set onlinexam schedule end */
	
	
	/* set schedule related function end */
  ...
  $scope.selectModules = function(){
		dataFactory.httpRequest('dashboard/selectModules','POST',{},{"classes":$scope.form.lpClasses,"subject":$scope.form.subject,"yearlevel":$scope.form.yearlevel,"gradelevel":$scope.form.gradelevel}).then(function(data) {
			 $scope.modules = data.modules;
		});
	}
	
  $scope.levelpathlevel = function(){
      dataFactory.httpRequest('dashboard/levelpathlevel','POST',{},{"classes":$scope.form.lpClasses}).then(function(data) {
          $scope.gradelevels = data;
      });
  }
  ...
  $scope.upload = function(id){
      ...
      dataFactory.httpRequest('assignments/checkUpload','POST',{},{'assignmentId':id}).then(function(data) {
        ...
        if(data.canApply && data.canApply == "true"){
            $scope.form.assignmentId = id;
			$scope.views.viewAssignment = true;
			$scope.views.upload = true;
           // $scope.changeView('upload');		
			$scope.fileupload="";
        }
      });
  ...
  $scope.remove = function(item,index){
    ...
    if (confirmRemove == true) {
      showHideLoad();
      dataFactory.httpRequest('learnPath/delete/'+item.id,'POST').then(function(data) {
        response = apiResponse(data,'remove');
        if(data.status == "success"){
          $scope.learnpaths.splice(index,1);
        }
        showHideLoad(true);
      });
    }
  ...
  $scope.saveAdd = function(){
    showHideLoad();
    dataFactory.httpRequest('learnPath','POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'add');
	  
      if(data.status == "success"){
        $scope.learnpaths.push(response);
        $scope.changeView('list');
      }
      showHideLoad(true);
    });
  ...
  $scope.edit = function(id){
    showHideLoad();
    dataFactory.httpRequest('learnPath/'+id).then(function(data) {
      $scope.changeView('edit');
      $scope.form = data;
	  $scope.classes = data.classes;
	  $scope.gradelevels = data.gradelevels;
	  $scope.subject = data.subjectsname;
	  $scope.modules = data.modules;
      showHideLoad(true);
    });
  }
	$scope.lpid="";
  $scope.viewLearnPath = function(id){
    showHideLoad();
    dataFactory.httpRequest('learnPath/viewLearnPath/'+id).then(function(data) {
      $scope.changeView('learnPathSet');
      $scope.form = data;
	  $scope.lpid=id;
	  $scope.subjectsname = data.subjectsname;
      showHideLoad(true);
    });
  }
  ...
  $scope.saveEdit = function(){
    ...
    dataFactory.httpRequest('learnPath/'+$scope.form.id,'POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'edit');
      if(data.status == "success"){
        $scope.learnpaths = apiModifyTable($scope.learnpaths,response.id,response);
        $scope.changeView('list');
      }
      showHideLoad(true);
    });
  ...
  $scope.marksData = function(id){
    showHideLoad();
    dataFactory.httpRequest('learnPath/marks/'+id).then(function(data) {
      $scope.marks = data.grade;
      $scope.examDegreeSuccess = data.examDegreeSuccess;
  ...
  $scope.viewGradeFeedback = function(id){
      ...
      dataFactory.httpRequest('assignments/viewGradeFeedback/'+id).then(function(data) {
          $scope.gradeFeedback = data.assignments;
          $scope.changeView('viewAssgnGradeFeedback');
          showHideLoad(true);
      });
  }
  $scope.changeView = function(view){
	...
	$scope.views.scheduleitemlist = false;
	$scope.views.scheduleAssgnlist = false;
	$scope.views.AssgnSchedule = false;
	$scope.views.scheduleOnexlist = false;
	$scope.views.OnlineexamSchedule = false;
	$scope.views.viewAssignment = false;
	$scope.views.editAssignment = false;
	$scope.views.editOnlineExam = false;
	$scope.views.moduleEdit = false;
	$scope.views.learnPathSet = false;
	$scope.views.viewAssgnGradeFeedback = false;
...
schoex.controller('ModulesController', function(dataFactory,$scope,$rootScope) {
  $scope.classes = {};
  $scope.modules = {};
  $scope.onlineexams = {};
  $scope.assignments = {};
  ...
  $scope.views.list = true;
  $scope.form = {};
  $scope.form.modulesetlist = [];
  $scope.form.assignmentsList = [];
  $scope.form.onlineexamsList = [];
  $scope.form.examQuestion = [];
  $scope.userRole ;
  $scope.yearlevels = {};
  $scope.gradelevels = {};
  ...
  $scope.showModal = false;
  dataFactory.httpRequest('modules/listAll').then(function(data) {	
    $scope.classes = data.classes;
	$scope.modules = data.modules;
	// $scope.onlineexams = data.onlineExams;
	// $scope.assignments = data.assignments;
	...
	$scope.checkToggle = function(){
	   $scope.albumNameArray = [];
		...
		if($scope.albumNameArray.length > 0){
				$scope.form.mdClasses=$scope.albumNameArray;
		}
		else{
			$scope.form.mdClasses="";
			return false;
		}
    }
  ...
  $scope.remove = function(item,index){
    ...
    if (confirmRemove == true) {
      showHideLoad();
      dataFactory.httpRequest('modules/delete/'+item.id,'POST').then(function(data) {
        response = apiResponse(data,'remove');
        if(data.status == "success"){
  ...
  $scope.asexsubjectList = function(responseFrom){
   ...
   $scope.subjectList = function(){
	...
	// $scope.editAssignSort = function(item,index){
	   // var value =$("#edassorder-"+item.assId).val();
	   // alert(item.assId);
       // item.assorder = value;	
	// }
	$scope.editAssignSort = function(item,index,rowindex){
  ...
  $scope.removeQuestion = function(question,index){
	...
	/* Assignment And OnlineExam related Function End*/
	
  $scope.saveAdd = function(){
    showHideLoad();
    dataFactory.httpRequest('modules','POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'add');
      if(data.status == "success"){
  ...
  $scope.addAssignmentData = function(){
    ...
    assignmentData.assId = $scope.form.assignment;
    assignmentData.assreq = $scope.form.requirement;
	//assignmentData.assorder = $scope.form.sortorder;$scope.form.modulesetlist
	assignmentData.assorder = $scope.form.modulesetlist.length;
...
schoex.controller('promotionController', function(dataFactory,$rootScope,$scope) {
  ...
  $scope.classesList = function(){
      dataFactory.httpRequest('dashboard/allclassesList','POST',{}).then(function(data) {
          $scope.classes = data;
      });
  }

  $scope.yearlevelList = function(){
      dataFactory.httpRequest('dashboard/yearlevelList','POST',{}).then(function(data) {
          $scope.yearlevel = data;
      });
  }
  
  $scope.classesPromoteList = function(key){
      dataFactory.httpRequest('dashboard/allclassesList','POST',{},{"academicYear":$scope.studentsList.students[key].acYear}).then(function(data) {
  ...
  $scope.listStudents = function(){
    ...
    dataFactory.httpRequest('promotion/listStudents','POST',{},$scope.form).then(function(data) {
	  ...
	  $scope.yearlevelList = data.yearlevelList;
...
schoex.controller('expensesController', function(dataFactory,$rootScope,$scope,$route) {
  ...
  $scope.getTotal = function(key){
    ...
    for(var i = 0; i < $scope.expenses[key].length; i++){
        total += ($scope.expenses[key][i].expenseAmount);
    }
    return total;
  }
...
schoex.controller('LanguageFieldController', function(dataFactory,$scope,$rootScope) {
  $scope.langaugesFields = {};
  $scope.assignments = {};
  $scope.views = {};
  ...
  $scope.showModal = false;
  dataFactory.httpRequest('languagesField/listAll').then(function(data) {	
	$scope.langaugesFields = data.langaugesFields;
	showHideLoad(true);
  });
  
  $scope.saveField = function(){
    showHideLoad();
    dataFactory.httpRequest('languagesField','POST',{},$scope.form).then(function(data) {
      response = apiResponse(data,'add');
      if(data.status == "success"){
        $scope.changeView('add');
		$scope.langaugesFields = data.langaugesFields;
